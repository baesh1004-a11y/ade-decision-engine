...
def run(db_path: str = "datahub/market.db") -> None:
    ...
    try:
        ...
        if not recommendations:
            ...
            if selected_from_workbench and selected_code == selected_from_workbench:
                st.success(f"통합 추천 워크벤치에서 선택한 종목 {selected_label}이 연결되었습니다.")
            if not validated:
            ...
            st.markdown(f"**선택 종목:** {selected_label}")
        ...
        pending = [
            r for r in requests
            if r["status"] == "PENDING_APPROVAL" and str(r.get("source_run_id") or "") == current_run_id
        ]
        ...
        if old_pending_count:
            st.warning(f"이전 추천 실행에서 생성된 승인 대기 주문 {old_pending_count}건은 현재 목록에서 제외했습니다.")
        if not pending:
        ...
        if not history.empty:
            ...
            keep = [c for c in ["created_at", "source_run_id", "source_rank", "종목", "종목코드", "side", "quantity", "order_type", "limit_price", "status", "broker_order_id", "broker_message", "error_message"] if c in history.columns]
        ...
        if not executions.empty:
            ...
            keep = [c for c in ["captured_at", "broker_order_id", "종목코드", "side", "ordered_quantity", "filled_quantity", "filled_price", "status"] if c in executions.columns]
