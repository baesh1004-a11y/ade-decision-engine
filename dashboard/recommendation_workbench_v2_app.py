...
def run() -> None:
    ...
    with title_col:
        st.markdown(
            ...
            '<p>하나의 추천 실행 ID를 기준으로 추천·검증·주문을 연결합니다.</p></div>',
    ...
    try:
        ...
        with step2:
            _step_title(st, 2, "추천 근거 비교", "현재 차트와 과거 급등 직전 패턴을 같은 기준으로 비교합니다.")
            _comparison_panel(st, selected, current, historical, pattern, payload, profile.code)
        with step3:
            _step_title(st, 3, "추천 검증", "현재 추천 실행과 같은 run_id의 검증만 표시합니다.")
            _validation_panel(st, selected, validation)
        with step4:
            _step_title(st, 4, "주문 관리", "현재 추천 실행에서 생성된 주문만 집계합니다.")
...
def _render_context_banner(st, context) -> None:
    ...
    tone = "정상 연결" if validated in {0, context.recommendation_count} else "부분 검증"
    st.markdown(
        f'<div class="context-banner"><b>{tone}</b><span>run_id {context.run_id}</span>'
        f'<span>추천 {context.recommendation_count}개</span><span>검증 {validated}개</span>'
...
def _render_kpis(st, context, recommendations) -> None:
    buy_count = sum(1 for row in context.validations.values() if str(row.get("decision")) == "FINAL BUY")
    watch_count = sum(1 for row in context.validations.values() if str(row.get("decision")) == "BUY WATCH")
    ...
    cards = [
        ...
        ("매수 검토", f"{buy_count}개", "동일 run_id 검증"),
        ("관찰 종목", f"{watch_count}개", "동일 run_id 검증"),
...
def _selected_recommendation(st, recommendations, market):
    ...
    ticker = st.selectbox("분석 종목 선택", tickers, index=tickers.index(st.session_state[key]), format_func=lambda value: labels[value], key=f"workbench_select_{market}")
...
def _comparison_panel(st, selected, current, historical, pattern, payload, market) -> None:
    st.markdown(
    ...
    with compare_tab:
        if historical.empty or pattern is None:
            ...
            st.plotly_chart(build_pattern_compare_chart(current, historical, selected["symbol"], historical_label), use_container_width=True, config=CHART_CONFIG)
    metrics = st.columns(4, gap="small")
    values = [
        ("주봉 순위점수", float(selected["weekly_similarity"]), "%"),
        ("STO 유사도", float(selected["sto_similarity"]), "%"),
        ("STO 필터", 1, "PASS"),
...
def _validation_panel(st, selected, validation) -> None:
    if validation is None:
        st.markdown('<div class="validation-result"><span>검증 결과</span><strong>미검증</strong></div>', unsafe_allow_html=True)
        st.caption("현재 추천 실행과 같은 run_id의 검증 결과가 없습니다.")
        return
    ...
    checks = [
        ...
        ("STO 필터", "PASS"),
        ("시장 레이더", _status_text(float(validation.get("market_score", 0)))),
        ("업종 레이더", _status_text(float(validation.get("sector_score", 0)))),
        ("위험 관리", _risk_text(float(validation.get("risk_score", 0)))),
    ]
    st.markdown(f'<div class="validation-result"><span>검증 결과</span><strong>{label}</strong></div>', unsafe_allow_html=True)
    for title, value in checks:
        st.markdown(f'<div class="validation-row"><b>{title}</b><span>{value}</span></div>', unsafe_allow_html=True)


def _order_panel(st, selected, market, validation, context) -> None:
    eligible = validation is not None and str(validation.get("decision")) in {"FINAL BUY", "BUY WATCH"}
    ...
    st.caption(f"연결 run_id: {context.run_id}")
...
def _step_title(st, number, title, description):
    st.markdown(f'<div class="step-header"><span>{number}</span><div><b>{title}</b><small>{description}</small></div></div>', unsafe_allow_html=True)
