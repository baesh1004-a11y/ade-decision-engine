...
기본 대시보드는 추천 목록, 추천 시점의 현재·과거 120거래일 비교, STO 3계층, 검토 메모와 주문 후보를 하나의 작업 화면으로 연결합니다. 검토할 추천 실행을 고정할 수 있으며, 새 계산이 0건으로 끝나면 그 결과를 그대로 표시합니다. 차트 집중 모드, 검토 상태 필터, 저장 후 다음 미검토 이동을 지원하며 입력 중 메모는 같은 세션에서 유지합니다.

새 추천의 비교 자료와 실행 조건은 결과 저장 시 함께 보관합니다. 이후 시세 정정에도 같은 자료를 읽으며, 손상·누락이 있으면 표시를 중단합니다. 백그라운드 추천은 OS 작업 잠금을 이용해 중단 상태를 정리하고, 이미 저장된 완료 결과를 복원합니다.
...
CLI·API의 종합 판단과 일일 추천, 선택 종목 환경 검토는 공통 실행 기록에 단계별 상태와 결과를 저장합니다. API `/decision`은 기존 응답에 `run`을 추가하며, 선택적인 `idempotency_key`로 같은 요청의 저장 결과를 다시 받을 수 있습니다. 추천 순위와 주봉·STO 계산식은 유지합니다.

[구현 범위·저장 구조·검증 방법](docs/decision_desk_runtime.md)을 참고하세요. 아래 엔진 설계 기준은 전체 목표 구조이며, 모든 설계 항목이 이번 변경으로 완성된 것은 아닙니다.

## 현재 기준

```text
ADE v1.0.4 Integrated Decision Engine
