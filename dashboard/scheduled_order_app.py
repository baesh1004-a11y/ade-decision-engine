...
def run(db_path: str = "datahub/market.db") -> None:
    ...
    try:
        ...
        st.title("예약주문")
        st.caption("시간·가격 조건이 충족되면 기존 승인 대기 주문으로 전환됩니다. KIS로 직접 전송되지 않습니다.")
        ...
        with st.form("scheduled_order_form"):
            ...
            limit_price = st.number_input("지정가 (시장가 선택 시 무시)", min_value=0.0, value=0.0, step=1.0)
            ...
            if recurrence != "NONE":
                ...
                recurrence_end_at = datetime.combine(
                    recurrence_end_date, time(23, 59), tzinfo=ZoneInfo("Asia/Seoul")
                )
        ...
        if not pending:
            ...
            for row in pending:
                ...
                cols[2].write(f"{row['order_type']} · {row.get('recurrence') or 'NONE'}")
                cols[3].write(trigger)
                if cols[4].button("재실행", key=f"run_schedule_{row['schedule_id']}"):
                    try:
        ...
        if not failed:
            st.caption("실패한 예약주문이 없습니다.")
        else:
            for row in failed:
                cols = st.columns([2, 3, 1])
                cols[0].write(f"**{row['ticker']} · {row['side']} {row['quantity']}주**")
        ...
        if not history:
            ...
            columns = [
                "schedule_id", "market", "trigger_type", "trigger_price", "scheduled_at",
                "ticker", "name", "side", "quantity", "order_type", "limit_price",
                "recurrence", "activation_count", "status", "generated_request_id",
