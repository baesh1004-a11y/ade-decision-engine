...
def apply_premium_theme(st, *, page: str = "ADE") -> None:
    st.markdown(
        f"""
        <style>
        :root{{--ade-navy:#0a2340;--ade-blue:#2468d8;--ade-cyan:#dff2ff;--ade-ink:#17324d;--ade-muted:#6f8193;--ade-line:#dce7f1;--ade-soft:#f4f8fb;--ade-glass:rgba(255,255,255,.90);--ade-positive:#18765a;--ade-warning:#b16a18;--ade-danger:#b84646;--ade-shadow:0 18px 46px rgba(31,73,110,.10)}}
        ...
        [data-testid="stSidebar"] a:hover{{background:rgba(230,242,255,.75)!important;color:#1f66ad!important}}
        ...
        .ade-hero{{display:flex;justify-content:space-between;align-items:flex-end;gap:18px;padding:24px 28px;border-radius:26px;background:linear-gradient(135deg,rgba(255,255,255,.97),rgba(238,247,255,.90));border:1px solid var(--ade-line);box-shadow:var(--ade-shadow);margin-bottom:16px;backdrop-filter:blur(14px)}}
        ...
        .ade-card{{padding:17px 18px;border:1px solid var(--ade-line);border-radius:18px;background:var(--ade-glass);box-shadow:0 9px 28px rgba(44,85,122,.07);backdrop-filter:blur(12px)}}
        .ade-card:hover{{transform:translateY(-1px);box-shadow:0 16px 34px rgba(44,85,122,.10)}}
        ...
        .ade-decision-grid{{display:grid;grid-template-columns:1.2fr .8fr .8fr;gap:12px}}.ade-decision-card{{padding:18px;border-radius:20px;background:linear-gradient(145deg,rgba(255,255,255,.97),rgba(239,247,255,.90));border:1px solid var(--ade-line);box-shadow:0 12px 34px rgba(37,78,116,.08)}}.ade-decision-card span{{display:block;color:var(--ade-muted);font-size:11px;letter-spacing:.08em;font-weight:800}}.ade-decision-card strong{{display:block;font-size:28px;margin-top:5px;color:var(--ade-navy)}}
        .ade-badge{{display:inline-flex;align-items:center;gap:6px;padding:6px 10px;border-radius:999px;font-size:11px;font-weight:850;background:#e9f3ff;color:#246bb0;border:1px solid rgba(36,104,216,.12)}}
        .ade-progress{{height:8px;border-radius:999px;background:#e8eef5;overflow:hidden}}.ade-progress>span{{display:block;height:100%;background:linear-gradient(90deg,#4b8be7,#58b7ef);border-radius:999px}}
        div[data-testid="stMetric"]{{background:rgba(255,255,255,.90);border:1px solid var(--ade-line);padding:15px 17px;border-radius:18px;box-shadow:0 9px 26px rgba(56,100,139,.07)}}
        ...
        [data-testid="stTabs"] button{{font-weight:780;color:#5f7388}}[data-testid="stTabs"] button[aria-selected="true"]{{color:#1d68b7}}
        .ade-positive{{color:var(--ade-positive)}}.ade-warning{{color:var(--ade-warning)}}.ade-danger{{color:var(--ade-danger)}}
        @media(max-width:900px){{.block-container{{padding:.75rem}}.ade-hero{{display:block;padding:21px}}.ade-chip{{display:inline-block;margin-top:12px}}.ade-status-grid{{grid-template-columns:repeat(2,minmax(0,1fr))}}.ade-decision-grid{{grid-template-columns:1fr}}}}
        </style>
        """,
...
def render_status_grid(st, items: list[tuple[str, str]]) -> None:
    cells = "".join(f"<div><span>{label}</span><strong>{value}</strong></div>" for label, value in items)
    st.markdown(f'<div class="ade-status-grid">{cells}</div>', unsafe_allow_html=True)
...
def render_decision_summary(
    ...
    *,
    ...
    st.markdown(
        f"""
        <div class="ade-decision-grid">
          <div class="ade-decision-card"><span>{title}</span><strong>{primary_value}</strong></div>
          <div class="ade-decision-card"><span>{secondary_label}</span><strong>{secondary_value}</strong></div>
          <div class="ade-decision-card"><span>{progress_label}</span><strong>{progress:.0f}%</strong><div class="ade-progress"><span style="width:{progress:.0f}%"></span></div></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
