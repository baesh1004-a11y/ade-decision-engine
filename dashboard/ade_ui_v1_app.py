...
def _render_live_orderbook(ticker: str, fallback_price: float) -> tuple[float,float,float]:
    client=_get_orderbook_client(ticker)
    snapshot=client.latest
    if snapshot is None:
        st.info("KIS 실시간 호가 연결 중입니다. 첫 호가 수신 전까지 계좌 현재가를 사용합니다.")
        return fallback_price,fallback_price,fallback_price
    rows=[]
    for level in reversed(snapshot.asks):
        rows.extend([f'<div class="ask">매도</div>',f'<div class="mid">{level.price:,.0f}</div>',f'<div>{level.quantity:,}</div>'])
    rows.extend(['<div class="head">구분</div>','<div class="head">가격</div>','<div class="head">잔량</div>'])
    for level in snapshot.bids:
        rows.extend([f'<div class="bid">매수</div>',f'<div class="mid">{level.price:,.0f}</div>',f'<div>{level.quantity:,}</div>'])
    ...
    st.caption(f"실시간 호가 · 매도총잔량 {snapshot.total_ask_quantity:,} · 매수총잔량 {snapshot.total_bid_quantity:,} · 수신 {time.strftime('%H:%M:%S',time.localtime(snapshot.received_at))}")
...
def _render_order_ticket(market: str,ticker: str) -> None:
    ...
    if account:
        c1,c2,c3=st.columns(3); c1.metric("주문가능 현금",f"₩{float(account.get('cash') or 0):,.0f}"); c2.metric("보유수량",f"{int(holding.get('quantity') or 0) if holding else 0}주"); c3.metric("현재가",f"₩{default_price:,.0f}" if default_price else "-")
    left,right=st.columns([1.2,1])
    with left:
        live_enabled=st.toggle("KIS 실시간 10호가",value=st.session_state.ade_live_orderbook,key="ade_live_orderbook_toggle",disabled=market!="kr" or not kis_configured())
        st.session_state.ade_live_orderbook=live_enabled
        if market=="kr" and live_enabled and kis_configured(): best_ask,best_bid,midpoint=_render_live_orderbook(ticker,default_price)
        else:
            best_ask=best_bid=midpoint=default_price
            st.info("실시간 호가가 꺼져 있어 계좌 스냅샷 현재가를 사용합니다.")
        if live_enabled and market=="kr":
            st.caption("호가 새로고침은 Streamlit 재실행 주기에 반영됩니다.")
    with right:
        ...
        price=st.number_input("가격",min_value=0.0,value=float(live_default or default_price),step=100.0,disabled=order_type=="MARKET")
        ...
        reference_price=price if order_type=="LIMIT" else float(midpoint or default_price)
        ...
        if st.button(f"{side} 주문 전송",type="primary",use_container_width=True,disabled=not can_submit):
            try:
                ...
                if result.accepted:
                    ...
                    st.session_state.ade_order_confirmation=False; _kis_data(refresh=True)
                else: st.error(f"KIS 모의주문 거절 · {result.message}")
            except Exception as exc: st.error(f"KIS 모의주문 전송 실패: {exc}")
...
def _render_status_bar() -> None:
    if kis_paper_enabled(): kis_text="KIS 모의투자 연결"; kis_class="ade-ok"
    elif kis_configured(): kis_text="KIS 설정 확인 필요"; kis_class=""
    else: kis_text="KIS 미설정"; kis_class=""
