...
- `RecordedPipeline`이 기존 종합 판단 엔진을 감싸 CLI·API 실행을 기록한다.
- `RunStateStore`의 실행·단계·JSON 산출물 저장, 전이 검사, 요청 중복 확인, 중첩 트랜잭션을 구현했다.
- 일일 추천과 종목별 환경 검토를 실행 기록에 연결했다. 개별 환경 검토는 다른 종목의 결과를 보존한다.
- 기본 UI를 Decision Desk로 전환하고, 추천 출처·검토 메모·한국/미국 주문 검토를 연결했다. 차트 집중 모드, 검토 대기열과 세션 초안 유지를 추가했다.
- 추천 결과와 비교 원천 자료·실행 조건을 함께 보관하고 무결성을 확인한다. 수동 백그라운드 추천은 OS 잠금에 근거해 중단 상태를 정리한다.
- 검증은 오프라인 엔진·API·Streamlit AppTest로 수행한다. 운영 계좌 연결과 실제 체결은 별도 검증 대상이다.

아래 표의 다른 항목은 기존 설계 기준 상태를 유지한다. 이번 변경의 정확한 범위와 남은 작업은 [런타임 구현 문서](docs/decision_desk_runtime.md)에 기록한다.
...
| 13 | Integration Orchestrator | 완료 | 기본 구현 | 통합 테스트 | 오프라인 확인 | CLI·API·추천·환경 검토 기록; 수동 추천 중단 상태 복구, 이어 실행은 후속 |
| 14 | Run State Store | 완료 | 기본 구현 | 트랜잭션·전이 테스트 | 오프라인 확인 | SQLite run/stage/artifact, 요청 중복 확인, JSON 무결성 |
| 15 | Configuration & Policy Engine | 완료 | 미구현 | 계획 완료 | 미확인 | 정책 버전, 승인, 실행별 불변 스냅샷 관리 |
| 16 | Data Snapshot & Lineage Engine | 완료 | 일부 | 비교 자료 무결성·롤백 테스트 | 오프라인 확인 | 추천 결과 저장 시 비교 자료 보관; 전체 엔진 입력 재현은 후속 |
