...
def _render_ai_confidence_card(st, selected: dict, ticker: str) -> None:
    ...
    score_text = str(score) if score is not None else "미확인"
    rows = "".join(
    ...
    st.markdown(
        ...
        <div class="confidence-card {tone}">
          <div class="confidence-head">
            ...
            <div class="confidence-score">{score_text}<span>{'점' if score is not None else ''}</span></div>
          </div>
          <div class="confidence-grid">{rows}</div>
          <div class="confidence-opinion"><strong>AI 종합 의견</strong><br>{opinion}</div>
          <div class="confidence-note">확인된 {sum(value != '미확인' for _, value, _ in factors)}/{len(factors)}개 항목만 반영했습니다. 투자 판단이나 주문 승인을 대신하지 않습니다.</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def _ai_confidence(selected: dict, radar) -> tuple[int | None, str, str, str, list[tuple[str, str, str]]]:
    ...
    if score is None:
        level, tone = "계산 불가", "neutral"
        opinion = "확인된 분석 데이터가 없어 신뢰도를 계산하지 않았습니다. 추천 검증을 먼저 실행하세요."
    elif score >= 80:
...
def _render_pending_approval(st, service, recommendations: list[dict]) -> None:
    ...
    if approve_col.button("승인하고 KIS로 전송", disabled=not confirm, type="primary"):
        try:
