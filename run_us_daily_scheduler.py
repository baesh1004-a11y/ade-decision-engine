...
HEARTBEAT_PATH = Path("output/us_core_status.json")
...
def _write_heartbeat(status: str, schedule: str, message: str = "") -> None:
    ...
    payload = {
        "status": status,
        "pid": os.getpid(),
        "updated_at": _now_et().isoformat(timespec="seconds"),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ADE US automatic post-close recommendations")
    ...
    if not DB_PATH.exists():
        raise SystemExit("datahub/us_market.db가 없습니다. 미국 가격·Replay DB를 먼저 구축하세요.")

    target_time = clock_time(args.hour, args.minute)
    schedule_text = f"US weekdays {args.hour:02d}:{args.minute:02d} America/New_York"
    print("========================================")
    print(" ADE US CORE")
    print("========================================")
    ...
    _write_heartbeat("RUNNING", schedule_text, "US ADE Core started")

    manager = ADEJobManager(
        lock_path="output/us_ade_job.lock",
        status_path="output/us_job_status.json",
    )
    ...
    try:
            ...
            if should_run:
                ...
                try:
                    if not service.auto_completed_today():
                        _write_heartbeat("RUNNING", schedule_text, "US AUTO recommendation queued")
                        with manager.acquire("US_AUTO_RECOMMENDATION", wait=True, timeout_seconds=6 * 60 * 60):
                            result = service.run(
                                "AUTO",
                                top_n=args.top,
                        ...
                        message = (
                            f"US AUTO completed: {result.recommendation_count} recommendations, "
                            f"{result.elapsed_seconds:.1f}s"
                        )
                        print(f"[{_now_et().isoformat(timespec='seconds')}] {message}")
                        _write_heartbeat("RUNNING", schedule_text, message)
                except Exception as exc:
                    message = f"US AUTO failed: {exc}"
                    print(f"[{_now_et().isoformat(timespec='seconds')}] {message}")
        ...
        _write_heartbeat("STOPPED", schedule_text, "US ADE Core stopped by user")
        print("US ADE Core stopped")
